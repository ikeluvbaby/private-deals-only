 ...
 * Webhook: HYDRA Google Apps Script — sends POST JSON
...
export default function BuyerList() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const HYDRA_WEBHOOK =
      "https://script.google.com/macros/s/AKfycbzNsjR4OUwd6pAGHitG3efvi3CihI_uA4WZGALGtxdXDdoi-21F20PO0QfbrX7bp14/exec";

    try {
      await fetch(HYDRA_WEBHOOK, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          source: "PRIVATE_DEALS_WEBSITE",
          Source: "PRIVATE_DEALS_WEBSITE",
      ...
      // no-cors responses are opaque — treat as success if no network error
    }
        ...
        <div className="container max-w-2xl mx-auto">
          ...
          {submitted ? (
            ...
            >
              ...
              >
                <div className="relative mt-0.5 shrink-0">
                  ...
                  <div
                    ...
                    style={{
                      border: consent
                        ? "1.5px solid oklch(0.71 0.115 73)"
                        : "1.5px solid oklch(0.35 0.006 60)",
                      background: consent ? "oklch(0.71 0.115 73 / 0.15)" : "transparent",
                      display: "flex",
                      alignItems: "center",
                  ...
                  >
                    {consent && (
                      <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
                        <path d="M1 4L3.5 6.5L9 1" stroke="oklch(0.71 0.115 73)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    )}
                  </div>
                </div>
                <span className="text-[oklch(0.50_0.007_75)] text-xs leading-relaxed">
                  I agree to the{" "}
                  <Link href="/privacy">
                    <span className="text-[oklch(0.71_0.115_73)] hover:text-[oklch(0.82_0.13_73)] transition-colors duration-200 underline underline-offset-2 cursor-pointer">
                      Privacy Policy
                    </span>
                  </Link>
                  {" "}and consent to being contacted regarding my submission.
                </span>
              </label>
              ...
              {/* Twilio contact helper */}
              <p
                className="text-[oklch(0.38_0.006_75)] text-xs text-center leading-relaxed"
                style={{ fontFamily: "'Inter', sans-serif" }}
              >
                Prefer to reach us directly? Leave a message or text us at{" "}
                <a
                  href="tel:+13656172009"
                  className="text-[oklch(0.72_0.12_75)] hover:text-[oklch(0.85_0.14_75)] transition-colors duration-200"
                >
                  (365) 617-2009
                </a>
                {" "}·{" "}
